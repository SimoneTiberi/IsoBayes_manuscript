...
do
	if [ -d $f ]; then
		...
		FalseDiscoveryRate -in $f/merge_index_percolator_pep.idXML -out $f/merge_index_percolator_pep.idXML -protein false -threads $NTHREADS -FDR:PSM $FDR -algorithm:add_decoy_peptides -algorithm:add_decoy_proteins
		IDScoreSwitcher -in $f/merge_index_percolator_pep.idXML -out $f/merge_index_percolator_pep_switched.idXML -new_score 'Posterior Error Probability_score' -new_score_orientation lower_better -new_score_type pep -threads $NTHREADS
		rm $f/merge_index_percolator_pep.idXML
	fi
done
