...
main = function(proteases){
  ...
  for (input in inputs) {
    ...
    for (protease in proteases) {
      ...
      plot_corr = scatterplot(abundances)
      ggsave(glue("{PATH_RES_ABUNDANCE}/{protease}/scatterplot_no_unique_{input}.png"), plot = plot_corr)
    ...
    plot_corr = scatterplot(abundances)
    ggsave(glue("{PATH_RES_ABUNDANCE}/scatterplot_no_unique_{input}.png"), plot = plot_corr)
    ...
    write.csv(SumTab_corr_no_unique, file = glue("{PATH_RES_ABUNDANCE}/correlation_no_unique_{input}.csv"), row.names = FALSE)
  }
