  ...
  for (model in c("psm", "intensities")) {
    ...
    for (no_protease in no_proteases) {
      ...
      if(model == "intensities"){
        data_list = IsoBayes:::build_intensity(data_list$x,
                                               glue("{no_protease}/AllPeptides.psmtsv"),
                                               variables)
      }else{
        data_list$y = data_list$x$`PSM Count (unambiguous, <0.01 q-value)`
      }
      PEPTIDE_DF = IsoBayes:::build_peptide_df(data_list$y, data_list$x)
      rm(data_list)
      PEPTIDE_DF$n_proteins_per_peptide = sapply(strsplit(PEPTIDE_DF$EC, split = "\\|"), length)
      PEPTIDE_DF_prot = PEPTIDE_DF[PEPTIDE_DF$n_proteins_per_peptide == 1, ]
      rm(PEPTIDE_DF)
      PEPTIDE_DF_prot$Y[PEPTIDE_DF_prot$QValue > 0.01] = 0 # FDR > 1%
      
      VALIDATION_DF_prot = build_validation(Y = PEPTIDE_DF_prot$Y,
                                            var = PEPTIDE_DF_prot$EC,
                                            var_name = "proteins"
                                            )
      if(model == "intensities"){
      ...
      VALIDATION_DF_prot = merge(VALIDATION_DF_prot, tpm_df,
                                 by.x = "proteins", by.y = "isoname",
                                 all.x = TRUE
                                 )
    ...
    for (no_protease in no_proteases) {
      ...
      PEPTIDE_DF = IsoBayes:::build_peptide_df(data_list$y, data_list$x)
      rm(data_list)
      EC_gene = lapply(strsplit(PEPTIDE_DF$EC, split = "\\|"),
                       function(x){gsub("-.*", "", x)}
                       )
