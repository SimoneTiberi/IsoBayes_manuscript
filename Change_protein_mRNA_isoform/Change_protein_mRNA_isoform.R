...
main = function(proteases, no_unique = FALSE){
  ...
  for (input in inputs) {
    for (mrna in c("", "_mRNA")) {
      ...
      for (protease in proteases) {
        ...
        if(mrna == ""){
          load(glue("{PATH_RES_MODEL}/{input}{mrna}/{protease}/{input}_MCMC.RData"))
        ...
        ggsave(glue("{PATH_RES_CHANGE}/{protease}/main_result_{input}{mrna}{no_unique_nm}.png"), plot = plot_change)
        
        plot_change = plot_prob_change(validation_dat_extreme)
        ggsave(glue("{PATH_RES_CHANGE}/{protease}/main_result_extreme_{input}{mrna}{no_unique_nm}.png"), plot = plot_change)
        ...
        ggsave(glue("{PATH_RES_CHANGE}/{protease}/scatterplot_log2FC_{input}{mrna}{no_unique_nm}.png"), plot = plot_scatter)
        ...
        ggsave(glue("{PATH_RES_CHANGE}/{protease}/scatterplot_probInc_log2FC_{input}{mrna}{no_unique_nm}.png"), plot = plot_scatter)
      }
      ...
      ggsave(glue("{PATH_RES_CHANGE}/main_result_{input}{mrna}{no_unique_nm}.png"), plot = plot_change)
      
      plot_change = plot_prob_change(benchmark_df_all_extreme)
      ggsave(glue("{PATH_RES_CHANGE}/main_result_extreme_{input}{mrna}{no_unique_nm}.png"), plot = plot_change)
      ...
      plot_scatter = scatterplot(benchmark_df_all[sel, c("Log2_FC", "Log2_FC_validation")]) + 
        labs(x = "Log2_FC", y = "Log2_FC_validation")
      ggsave(glue("{PATH_RES_CHANGE}/scatterplot_log2FC_{input}{mrna}{no_unique_nm}.png"), plot = plot_scatter)
      ...
      plot_scatter = scatterplot(benchmark_df_all[sel, c("Prob_prot_inc", "Log2_FC_validation")]) + 
        labs(x = "Prob_prot_inc", y = "Log2_FC_validation")
      ggsave(glue("{PATH_RES_CHANGE}/scatterplot_probInc_log2FC_{input}{mrna}{no_unique_nm}.png"), plot = plot_scatter)
    }
  }
  ...
  for (protease in proteases) {
    ...
    for (input in c("MM_psm", "OpenMS")) {
      # load validation data with res
      load(glue("{PATH_RES_MODEL}/{input}_mRNA/{protease}/Merged_validation_res_{input}_mRNA"))
      colnames(validation_dat)[grep("tpm", colnames(validation_dat))] = "tpm_validation"
